...
public class Tuple {
    ...
    public String toString() {
       // some code goes here
    	StringBuffer field_String = new StringBuffer(); // Buffer String to store tuple.
    	for(int i=0; i<fields.length; i++)
    	{
    		if(fields.length == 0) // if field has no value.
    			throw new UnsupportedOperationException("Implement this");
    		else if(i == fields.length-1) // if number of fields reach to length then | will no add to string. 
    			field_String.append(fields[i]);
    		else
    			field_String.append(fields[i] + "|"); 
    	}
    	return field_String.toString(); // Return string toString method is of StringBuffer class.
    }
}
