...
public class HeapFile implements DbFile {
	...
	public Page readPage(PageId pid) {
		...
		try {
			...
			int temp = BufferPool.PAGE_SIZE * pid.pageno();
			byte[] tuples_data = new byte[BufferPool.PAGE_SIZE];
			file.seek(temp);
			file.readFully(tuples_data);
			file.close();
			return new HeapPage((HeapPageId) pid, tuples_data);
		} catch (Exception e) {
			e.getMessage();
		...
		// int offset = this.page.getId().pageno();
	...
	public DbFileIterator iterator(TransactionId tid) {
		...
		return new DbFileIterator() {
			...
			public void open() throws DbException, TransactionAbortedException {
				// TODO Auto-generated method stub
				currentPage = Database.getBufferPool().getPage(tid, this.hp.pid, null);
			...
			public boolean hasNext() throws DbException, TransactionAbortedException {
				...
				if (this.tuples != null) {
					...
					if (this.tuples.hasNext()) {
						...
						if (this.currentPageNo < HeapFile.this.numPages()) {
							this.currentPageNo++;
