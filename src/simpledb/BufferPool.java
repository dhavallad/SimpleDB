...
public class BufferPool {
	...
	public Page getPage(TransactionId tid, PageId pid, Permissions perm)
		...
		// some code goes here
		if (pages.get(pid) != null) { // If page found then return that page.
			return pages.get(pid);
		} else { // When no page found from the disk.
			if (pages.size() >= numPages) {
				throw new DbException("Error!!Page not found on the disk.");
			}
	 ...
	 * is not needed for assignment2). May block if the lock cannot be acquired.
	 * 
