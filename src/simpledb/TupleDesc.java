...
public class TupleDesc {
	...
	public String toString() {
		...
		for(int i = 0 ; i < this.numFields(); i++){
			if(this.numFields() == 0)	// if field has no value.
    			throw new UnsupportedOperationException("Field has no value.");
			else if(i == this.numFields()-1) // If number of fields reach to length then | will no add to string. 
				string1.append(getType(i)).append("(").append(getFieldName(i)).append(")");
    		else	//	Remain possibility - append string with | to string1.
    			string1.append(getType(i)).append("(").append(getFieldName(i)).append(")").append("|");
    	}
